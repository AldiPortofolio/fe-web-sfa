...
import { ModalDownload, LoadingDots } from '.'
...
class BulkCreate extends React.Component {
  ...
  componentDidMount(){
    const { auth: {access_token}, actionUrl } = this.props
    axios.defaults.headers.common['Authorization'] = `Bearer ${access_token}`

    axios.get(NEWAPI_V2 + `/${actionUrl}/download-template`)
    .then(data => {
  ...
  uploadBulk = () => {
    ...
    if (this.refs.file.files) {
      ...
      axios.post(NEWAPI_V2 + `/${actionUrl}/bulk`, data)
      .then(data => {
        let result = data.data;
        if(result.meta.status === false){
          this.setState({confirmIsOpen: true, type: 'error', textError: result.meta.message + ' Silahkan ubah dan upload ulang file.' , uploadStatus: 'Upload File', error_url: result.data.error_file})
        }else{
