...
import generate from '../icon-callplan-active.png';
import cross from '../x.png';

const ModalAction = ({confirmIsOpen, resultIsOpen, type, confirmClose, resultClose, confirmYes, confirmText, resultText, buttonActionText}) => {
  return (
    <div>
      <Modal className="modal-confirmation d-flex align-items-center justify-content-center" size='sm' isOpen={confirmIsOpen} toggle={() => confirmClose()}>
        <ModalBody>
          <div className="d-flex flex-column align-items-center align-items-center">
            <img src={generate} alt='check' style={{width: '150px', marginTop: '20px'}} />
            <div style={{margin: '35px 0 15px'}}>
              { (confirmText && confirmText.length > 0) ? confirmText : "Apakah Anda yakin untuk action ini?" }
            </div>
          </div>
        </ModalBody>
        <ModalFooter className='text-center'>
          <button className="btn btn-outline-danger btn-rounded btn-block mr-2" onClick={() => confirmClose()}>Cancel</button>
          <button className="btn btn-danger btn-rounded btn-block ml-2" onClick={() => confirmYes()}>{buttonActionText}</button>
        </ModalFooter>
      </Modal>
