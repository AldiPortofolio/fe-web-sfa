...
class BulkAssignment extends React.Component {
  state = {
    confirmIsOpen: false,
    TemplateBulk: '',
    type: '',
  ...
  componentDidMount(){
    ...
    axios.get(`/sales_management/download_template`)
    .then(data => {
  ...
  uploadSalesAssigmentBulk = () => {
    if (this.refs.file.files) {
      ...
      axios.post('sales_management/bulk_assignment', data)
      .then(data => {
        let result = data.data;
        if(result.meta.status === false){
          this.setState({confirmIsOpen: true, type: 'error', textError: result.meta.message, uploadStatus: 'Upload'})
        }else{
          this.setState({confirmIsOpen: true, type: 'success', textSuccess: 'Bulk assignment sales sukses', uploadStatus: 'Upload'})
        }
      })
      .catch(e => {
        this.setState({confirmIsOpen: true, type: 'error', textSuccess: 'Bulk assignment sales gagal', uploadStatus: 'Upload'})
      })
    }
  }
  ...
  render() {
    ...
    const { TemplateBulk, confirmIsOpen, type, textSuccess, textError, upload, uploadStatus, language } = this.state
