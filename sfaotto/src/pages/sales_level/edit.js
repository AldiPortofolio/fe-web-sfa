...
import { createSalesLevel, detailSalesLevel, editSalesLevel } from '../../actions/sales_level';
...
class Edit extends React.Component {
  ...
  componentWillMount() {
    const { auth: {access_token}, detailSalesLevel, match } = this.props
    axios.defaults.headers.common['Authorization'] = `Bearer ${access_token}`

    detailSalesLevel(match.params.id)
    .then((data) => {
      const all = data.data.sales_level
      this.setState({
        ...all
      })
    })
  ...
  componentDidMount(){
    document.title = "SFA OTTO - Edit Sales Level"
  ...
  render() {
    const { auth, history, editSalesLevel } = this.props
    const {
      id,
      name,
      total_merchant_visit,
...
export default connect(
  ({auth}) => ({ auth }),
  {createSalesLevel, detailSalesLevel, editSalesLevel}
)(Edit)
