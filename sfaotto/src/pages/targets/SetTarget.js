...
class SetTarget extends React.Component {
  ...
  settedValue = (props) => {
    ...
    if(auth.province_id !== null){
      this.setState({selectedProvince: {value: auth.province_id, label: auth.province_name}});

      getCities(auth.province_id)
    }

    if(auth.city_id !== null){
      this.setState({selectedCity: {value: auth.city_id, label: auth.city_name}});

      getDistricts(auth.city_id)
    }

    if(auth.district_id !== null){
      this.setState({selectedDistrict: {value: auth.district_id, label: auth.district_name}});
          ...
          <div className="col-12">
            <nav className="d-flex justify-content-end" aria-label="breadcrumb">
              <ol className="breadcrumb">
                ...
                <li className="breadcrumb-item active" aria-current="page">Set Target</li>
              </ol>
            </nav>
          </div>
            ...
            <div className="accordion mb-4" id="target-accordion">
              {(['IT Admin', 'Manager', 'ASM'].includes(auth.role)) &&
                <div className="card mb-4">
                  <div onClick={() => this.setCollapse('provinsi')} className={`card-header ${(collapse === 'provinsi') ? '' : ''}`} data-toggle="collapse">
