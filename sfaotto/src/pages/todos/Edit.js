...
import { NotAuthorize, ModalConfirm, SelectLineComponent, LoadingSpinner } from '../../components'
...
class Register extends React.Component {
  ...
  componentWillMount() {
    const { auth: {access_token}, getTodo } = this.props
    axios.defaults.headers.common['Authorization'] = `Bearer ${access_token}`
    getTodo(this.props.match.params.id)
      .then(({data: {todo_list}}) => {
        let selectedCategory = find(this.state.todoCategories, {label: todo_list.category})
        this.setState({
          ...todo_list,
          selectedCategory
        })
  ...
  render() {
    let allFilled = false
    const { auth, sales, editTodo, history } = this.props
    const { 
      id,
      todoCategories,
      upload,
                ...
                <form
                  onSubmit={(e) => {
                    ...
                    if (selectedCategory.value === 0) {
                      editTodo(id, {sales_phone, category: selectedCategory.value, merchant_name, merchant_address, merchant_phone, message })
                        .then(data => this.setState({confirmIsOpen: true, type: 'success', textSuccess: 'Tambah To-do List sukses'}))
                        .catch(e => this.setState({confirmIsOpen: true, type: 'error', textError: e.message}))
                    }else if (selectedCategory.value === 1) {
                      editTodo(id, {sales_phone, category: selectedCategory.value, merchant_id, message })
                        .then(data => this.setState({confirmIsOpen: true, type: 'success', textSuccess: 'Tambah To-do List sukses'}))
                        .catch(e => this.setState({confirmIsOpen: true, type: 'error', textError: e.message}))
                    }else{
                      editTodo(id, {sales_phone, category: selectedCategory.value, message})
                        .then(data => this.setState({confirmIsOpen: true, type: 'success', textSuccess: 'Tambah To-do List sukses'}))
                        .catch(e => this.setState({confirmIsOpen: true, type: 'error', textError: e.message}))
                    }
                  }}
                  ...
                  <div className="row">
                    ...
                    <div className="col-12 col-lg-6">
                      <div className="form-group">
                        ...
                        <input onChange={e => this.setState({sales_name: e.target.value})} value={sales_name} type="text" name="sales-name" className="form-control form-control-line" placeholder="Nama Sales" />
                      </div>
                    </div>
                    <div className="col-12 col-lg-6">
                      <div className="form-group">
              ...
              <div className="card-body text-center text-md-left">
                <div className="row">
                  ...
                  <div className="col-12 col-lg-4 d-flex align-items-center mt-4 mt-lg-0 justify-content-center justify-content-lg-end">
                    <Link to="/todos/manage" className="btn btn-rounded btn-danger">Manage To-do List</Link>
                  </div>
                </div>
              </div>
            </div>
                  ...
                  onSubmit={e => {
                    ...
                    if (this.refs.file.files) {
                      ...
                      axios.post('/todo_list/bulk', data)
                        .then(data => this.setState({confirmIsOpen: true, type: 'success', textSuccess: 'Bulk upload To-do List sukses'}))
                        .catch(e => this.setState({confirmIsOpen: true, type: 'error', textError: 'Bulk upload To-do List gagal'}))
                    }
                  }}
                  >
